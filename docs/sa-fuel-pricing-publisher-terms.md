# Separate SA Government fuel service: publisher terms and release controls
...
| 3.4 | No further dissemination after termination. | Stop refreshes; remove local files with the separate script's `--retire`, remove origin files, redeploy, purge caches and verify both public JSON URLs no longer work. The static asset URL is itself redistribution. |
...
The supplied agreement supports free publication with its conditions; the earlier note that redistribution terms were unknown is superseded. The former Pulse client previously implemented attribution, direct State contact, source separation and a freshness cutoff. That client is now retired. This independent project currently has no public page or active distribution. Its separate private ingestion script preserves a local termination helper. **Production is not enabled or confirmed compliant:** public distribution, deployment/update cadence, audience reports and public-link validation require a new approval. Do not promote local data to the public site until the operational tasks above have been completed. No changes were made to the provider's account or hosting.

This document belongs to the paused SA Government project, **not** the historical Pulse map. No State data is served by the revised Pulse page.
